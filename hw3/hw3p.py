...
def get_data(file):
	...
	return data
'''
file1 = os.path.join(userhome, 'Desktop', 'X_set3.csv')
file2 = os.path.join(userhome, 'Desktop', 'y_set3.csv')
file3 = os.path.join(userhome, 'Desktop', 'z_set3.csv')
'''
'''
file1 = os.path.join(userhome, 'Desktop', 'X_set3.csv')
file2 = os.path.join(userhome, 'Desktop', 'y_set3.csv')
...
file2 = os.path.join(userhome, 'Desktop', 'y_set3.csv')
file3 = os.path.join(userhome, 'Desktop', 'z_set3.csv')
x, y, z = get_data(file1), get_data(file2), get_data(file3)
x, y, z = np.array(x), np.array(y), [item[0] for item in z] 
...
#-----------------------judge converge------------------------------------------------
def VI(alpha_a, alpha_b, e, f, x, y, sigma, u, time):
	...
	if time < 120:
		...
		item6 = 0.5 * (math.log(np.linalg.det(sigma * 10)) - d * math.log(10))
...
EQLAM, ALPHA_A, ALPHA_B, E, F, SIGMA, UU = iterations(x, y, 500)

def VI_picture():
	result = []
	for i in range(500):
		result.append(VI(ALPHA_A[i], ALPHA_B[i], E[i], F[i], x, y, SIGMA[i], UU[i], i)[0])
	plt.plot(result)
	plt.show()
	return result
...
def Omega_picture(x, y, z):
	u = UU[499]
	predict_y = [np.dot(x[i], u) for i in range(N)]
	plt.plot(z, predict_y, color = "black")
	plt.scatter(z, y, color = "green")
	plt.plot(z, 10 * np.sinc(z), color = "red")
	plt.show()
